...
<?php
	...
	$req='SELECT c.numConfrontation, c.nomconfrontation, c.dateconfrontation FROM "FindMentor"."Confrontation" c, "FindMentor"."Joueur" j, "FindMentor"."Participation" p WHERE p.numconfrontation = c.numconfrontation and p.numjoueur = j.numjoueur and j.pseudojoueur =\''.$_COOKIE['pseudo'].'\';';
	$reponseConfrontation = $BDD->query($req);

	$req='SELECT COUNT(*) FROM "FindMentor"."Confrontation" c, "FindMentor"."Joueur" j, "FindMentor"."Participation" p WHERE p.numconfrontation = c.numconfrontation and p.numjoueur = j.numjoueur and j.pseudojoueur =\''.$_COOKIE['pseudo'].'\';';
	...
	if ($nbParticipation == 0) { echo '<p> Je suis désolé, vous ne vous êtes inscrit à aucune confrontation de ligues. Inscrivez-vous vite !</p>'; }
	else {
?>
